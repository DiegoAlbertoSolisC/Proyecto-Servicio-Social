...
void LXeDetectorConstruction::DefineMaterials()
{
  ...
  G4Element* elSi = new G4Element("Silicon", "Si", z=14, a=28.085* g / mole);  // Silicio, Z=14, A=28.085 g/mol
  G4Element* elO = new G4Element("Oxygen", "O", z=8, a=15.999* g / mole);      // Oxígeno, Z=8, A=15.999 g/mol
  //***Materials
  //Aerogel


  // Composición del aerogel (por ejemplo, 99% sílice y 1% aire)
  G4Material* aerogel = new G4Material("Aerogel", density=0.003, ncomponents=2);
  aerogel->AddElement(elSi, natoms=1);
  ...
  std::vector<G4double> rIndexAerogel = {1.03, 1.03};  // índice de refracción

  auto aerogel_mt = new G4MaterialPropertiesTable();
  aerogel_mt->AddProperty("RINDEX", photonEnergy, rIndexAerogel);

  // (opcional, absorción)
  //std::vector<G4double> absLength = {100. * cm, 100. * cm};
  //aerogel_mt->AddProperty("ABSLENGTH", photonEnergy, absLength);

  G4Material::GetMaterial("Aerogel")->SetMaterialPropertiesTable(aerogel_mt);
}
...
void LXeDetectorConstruction::SetDefaults()
{
  ...
  fNx = 120; // y ni siquiera es el limite
  fNy = 120;
  fNz = 0;
